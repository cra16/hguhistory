...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/xe_v3_default','_header.html') ?>
<?php if($__Context->oDocument->isExists()){;
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/xe_v3_default','_read.html');
} ?>
<?php if(!$__Context->document_list && !$__Context->notice_list){ ?><p class="no_ducument"><?php echo $__Context->lang->no_documents ?></p><?php } ?>
<div class="board_list" id="board_list">
	<?php if($__Context->document_list || $__Context->notice_list){ ?><table width="100%" border="1" cellspacing="0" summary="List of Articles">
		<thead>
			<!-- LIST HEADER -->
			<tr>
...
<div class="list_footer">
	...
	<div class="btnArea">
		...
		<?php if($__Context->grant->manager){ ?><span class="btn"><a href="<?php echo getUrl('','module','document','act','dispDocumentManageDocument') ?>" onclick="popopen(this.href,'manageDocument'); return false;"><?php echo $__Context->lang->cmd_manage_document ?></a></span><?php } ?>
	</div>
	...
	<?php if($__Context->grant->view){ ?><form action="<?php echo getUrl() ?>" method="get" onsubmit="return procFilter(this, search)" id="board_search" class="board_search" ><input type="hidden" name="act" value="<?php echo $__Context->act ?>" />
		...
		<span class="btn"><button type="submit" onclick="xGetElementById('board_search').submit();return false;"><?php echo $__Context->lang->cmd_search ?></button></span>
        <?php if($__Context->last_division){ ?><span class="btn"><a href="<?php echo getUrl('page',1,'document_srl','','division',$__Context->last_division,'last_division','') ?>"><?php echo $__Context->lang->cmd_search_next ?></a></span><?php } ?>
	</form><?php } ?>
...
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/xe_v3_default','_footer.html') ?>
