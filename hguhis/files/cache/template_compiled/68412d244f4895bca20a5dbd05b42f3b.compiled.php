...
<div class="xe">
	<div class="header">
		...
		<div class="language">
			<strong title="<?php echo $__Context->lang_type ?>"><?php echo $__Context->lang_supported[$__Context->lang_type] ?></strong> <button type="button" class="toggle"><img src="/git/hguhistory/hguhis/layouts/xe_official/images/<?php echo $__Context->layout_info->colorset ?>/buttonLang.gif" alt="Select Language" width="87" height="15" /></button>
			<ul class="selectLang">
...
if($__Context->key!= $__Context->lang_type){ ?><li><button type="button" onclick="doChangeLangType('<?php echo $__Context->key ?>');return false;"><?php echo $__Context->val ?></button></li><?php }} ?>
		...
		<form action="<?php echo getUrl() ?>" method="post" class="iSearch"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" />
			...
			<input type="image" src="/git/hguhistory/hguhis/layouts/xe_official/images/<?php echo $__Context->layout_info->colorset ?>/buttonSearch.gif" alt="<?php echo $__Context->lang->cmd_search ?>" class="submit" />
		</form>
	</div>
	<div class="body">
		<div class="lnb">
