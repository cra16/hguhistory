...
${'ipaddress13_argument'} = new Argument('ipaddress', $args->{'ipaddress'});
${'ipaddress13_argument'}->ensureDefaultValue($_SERVER['REMOTE_ADDR']);
if(!${'ipaddress13_argument'}->isValid()) return ${'ipaddress13_argument'}->getErrorMessage();
if(${'ipaddress13_argument'} !== null) ${'ipaddress13_argument'}->setColumnType('varchar');

${'isvalid14_argument'} = new Argument('isvalid', $args->{'isvalid'});
${'isvalid14_argument'}->ensureDefaultValue('N');
if(!${'isvalid14_argument'}->isValid()) return ${'isvalid14_argument'}->getErrorMessage();
if(${'isvalid14_argument'} !== null) ${'isvalid14_argument'}->setColumnType('char');
...
,new InsertExpression('`ipaddress`', ${'ipaddress13_argument'})
,new InsertExpression('`isvalid`', ${'isvalid14_argument'})
));
