...
$lang->about_reset_auth_mail='현재등록된 이메일 주소는 %s입니다. 이메일 주소를 변경하시고자 하는 경우 새로운 이메일 주소로 회원정보 갱신 후 인증메일을 재발 송할 수 있습니다.';
...
$lang->about_member_sync='회원정보와 게시물과 댓글 정보를 동기화 합니다. 데이터가 많은 경우 시간이 오래 소요될 수 있습니다. 이용자가 많은 경우 반드시 서비스를 중단하시고 진행하시기 바랍니다.';
$lang->msg_success_modify_email_address='이메일 주소가 정상적으로 변경되었습니다. 변경된 이메일 주소로 로그인 가능합니다.';
$lang->group='그룹';
$lang->retrieve_password='비밀번호 찾기';
$lang->excess_ip_access_count='로그인 가능 횟수를 초과하셨습니다. %s 간 로그인 하실 수 없습니다.';
$lang->enable_login_fail_report='계정 무한 대입 방지 사용';
$lang->login_fail_report='로그인 실패 기록 보고 입니다.';
$lang->login_fail_report_contents='<h2>로그인 실패 기록을 알려드립니다.</h2><div>%1$s</div><p>* 비밀번호를 틀리는 등의 일이 없었는데 이 메시지를 보신다면, 계정 관리에 유의해주시기 바랍니다.<br />* 이 메시지는 로그인이 성공한 순간 누적 로그인 실패 기록이 많을 경우, 로그인 성공 이전 실패 기록을 모아서 발송합니다.<br />발송 시각: %2$s</p>';
