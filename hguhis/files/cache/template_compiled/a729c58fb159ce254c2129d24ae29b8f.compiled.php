...
<?php Context::addJsFile("./files/ruleset/login.xml", false, "", 0, "head", true, "") ?><form id="fo_login_widget" action="<?php echo getUrl('','act','procMemberLogin') ?>" method="post"  class="login_<?php echo $__Context->colorset ?>"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="@login" />
	<fieldset>
...
echo $__Context->lang->email_address;
			...
			<?php if($__Context->colorset){ ?><input type="image" src="/git/hguhistory/hguhis/widgets/login_info/skins/xe_official/images/<?php echo $__Context->colorset ?>/buttonLogin.gif" alt="login" class="login" /><?php } ?>
